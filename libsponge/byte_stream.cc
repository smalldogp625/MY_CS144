...
ByteStream::ByteStream(const size_t capacity):_queue(),_written_size(0),_read_size(0),_capacity(capacity),_end_input(false) { DUMMY_CODE(capacity); }

size_t ByteStream::write(const string &data) {
    if(_end_input) return 0;
    size_t write_size = min(data.size(),_capacity - _queue.size());
